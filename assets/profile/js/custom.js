...
$("body").on('keyup', ".text_area", function(e) {
	...
	$(".text_area h1,.text_area h2,.text_area h3,.text_area h4,.text_area h5,.text_area h6").replaceWith(function () {
...
$("body").on('blur', ".text_area", function(e) {
	...
	$(".text_area svg").remove();
	$(".text_area a").wrap('<span contentEditable="false"></span>');
	$(".text_area a").attr('target', '_blank');
	$(".text_area h1,.text_area h2,.text_area h3,.text_area h4,.text_area h5,.text_area h6").replaceWith(function () {
	  return "<div>" + $(this).text() + "</div>";
	});
...
$('.text_area').afterPaste({
		...
		callback: function() {
			...
			value_input = value_input.replace(/(<[^>]+) style=".*?"/ig, '$1');
			value_input = value_input.replace(/(<[^>]+) class=".*?"/ig, '$1');
			value_input=value_input.replace(/<[^/>][^>]*><\/[^>]+>/gim, "");
			$('#'+input_rel1).val(value_input);
